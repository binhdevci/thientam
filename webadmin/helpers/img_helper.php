...
function fn_resize_image($src, $dest, $new_width = 0, $new_height = 0, $make_box = true, $bg_color = '#ffffff', $save_original = false)
{
	...
	if (file_exists($src) && !empty($dest) && (!empty($new_width) || !empty($new_height)) && extension_loaded('gd'))
	{
        ...
        */
		// doan nay lam thay doi extension file
...
function fn_show_banner($filename,$link='', $w='', $h='')
{
	...
	switch($ext){
		case "swf" :
			$img='<a href="'.$link.'" taget="_blank"><embed '.$h.' '.$w.' src="'.$filename.'" wmode="transparent"/></a>' ;
			break ;
		default :
			$img='<a href="'.$link.'" taget="_blank"><img src="'.$filename.'" border=0 '.$h.' '.$w.' class="imgbor" /></a>' ;
			break ;
	}
	return $img ;
}
