...
<table class="form">
	...
	<tr>
		...
		<td><input type="text" name="lb_title_background" id="lb_title_background"class="w350" value="<?=$rs->lb_title_background?>"></td>
	</tr>
	<tr>
                ...
                <tr>
                    <td><div id="list_song" name="list_song" style="width: 400px;height:300px; border:1px solid #999999;overflow:scroll;">
                            <table class="admindata" id="tbl_row_news" >
								...
								$k = 1;
								$i = 1 ;
								foreach($list_news as $r):
							?>

								<tr class="row<?=$k?> news-<?=$i?>">
									<td><input type="checkbox"  class="checkbox_id_news<?=$i?>" id="checkbox_id_news<?=$i?>" name="ar_id_news[]" checked="checked" value="<?=$r->id_news?>" >
										<input type="hidden" class="hdn_checkbox_id_news<?=$i?>" id="hdn_checkbox_id_news<?=$i?>" name="hdn_arrvote[]"  value="<?=$r->id_news?>">
										<input type="hidden" class="hdn_delete_checkbox_id_news<?=$i?>" id="hdn_checkbox_id_news<?=$i?>" name="hdn_delete_arrvote[]"  value="0"></td>
									<td class="w300"><?=$r->lb_title?></td>
...
<script>
	...
	function remove()
	{
		var leng_row=document.getElementById('tbl_row_news').rows.length;
		for(var i=1;i<=leng_row;i++)
		{
			var value=$(".checkbox_id_news"+i).val();
			var checked = $(".checkbox_id_news"+i).is(':checked'); 
			if(!checked){
				$(".song-"+i).remove();
			}
		}
	...
	{
		var leng_row=document.getElementById('tbl_row_news').rows.length;
		var checked = $("#sa_song").is(':checked'); 
		if(checked){
			 for(var i=1;i<=leng_row;i++)
             {
                $(".checkbox_id_news"+i).attr('checked',true); 
				$(".hdn_checkbox_id_news"+i).val(1);
             }
		}else{
			 for(var i=1;i<=leng_row;i++)
             {
			 	$(".checkbox_id_news"+i).attr('checked',''); 
				$(".hdn_checkbox_id_news"+i).val(0);
			 }
		}
